...
public class CompanyTest extends JFrame implements ActionListener{
 ...
 public CompanyTest(){
   ...
   public void actionPerformed(ActionEvent e){
        if(e.getActionCommand().equals("display")){
            info=new JDialog(this,"Data from database",true);
            ...
            List<Child> children = company.selectChildren();
            dane.setText(createResultFromDataBase(children));
        ...
        if(e.getActionCommand().equals("add")){
            if(formValidation()) {
                ...
                company.insertToChildren(name, dateOfBirth, parent_one, parent_two);
            }
        }
        if(e.getActionCommand().equals("searchName")){
            if(searchNameValidation()) {
                ...
                scroll.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
                info.getContentPane().add(scroll);
                List<Child> children = company.selectChildByName(name);
                dane.setText(createResultFromDataBase(children));
                info.setVisible(true);
                info.setResizable(false);
            }
       ...
       if(e.getActionCommand().equals("searchDateOfBirth")){
           if(searchDateOfBirthValidation()) {
               ...
               scroll.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
               info.getContentPane().add(scroll);
               List<Child> children = company.selectChildByDateOfBirth(dateOfBirth);
               dane.setText(createResultFromDataBase(children));
               info.setVisible(true);
               info.setResizable(false);
           }
   ...
   public String createResultFromDataBase(List<Child> children){
       String text = "";
       id = 0;
       for(int i = 0; i<children.size(); i++){
           id++;
           if((children.get(i).getParent1id() == 0)||(children.get(i).getParent2id() == 0)){
               if(children.get(i).getParent1id() == 0){
                   text=text+id+" "+children.get(i).getName()+" "+children.get(i).getDateOfBirth()+" NULL "+children.get(i).getParent2id()+"\n";
               }
               else{
                   text=text+id+" "+children.get(i).getName()+" "+children.get(i).getDateOfBirth()+" "+children.get(i).getParent1id()+" NULL"+"\n";
               }
           }
           else{
               text=text+id+" "+children.get(i).getName()+" "+children.get(i).getDateOfBirth()+" "+children.get(i).getParent1id()+" "+children.get(i).getParent2id()+"\n";
           }
       }
       return text;
   }
